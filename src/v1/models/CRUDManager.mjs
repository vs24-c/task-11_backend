class CRUDManager {
  ...
  async getList(filters = {}, projection = null, populateFields = []) {
    try {
      // Создаем запрос на поиск с применением фильтров и проекции      
      let query = this.model.find(filters, projection);
      
        // Перебираем поля для популяции и добавляем их к запросу
        // populateFields.forEach((field) => {
        //   if (typeof field === 'string') {
        //     // Если поле передано как строка, используем его напрямую для populate
        ...
        //   ) {
        //     // Если передан объект с параметрами для populate, применяем их
